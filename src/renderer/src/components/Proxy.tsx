...
function ProxyComponent(): JSX.Element {
    ...
    const handleChooseProxy = async (proxy: Proxy): Promise<void> => {
        if (currProxy) {
            alert('Please disconnect from current proxy first.')
            return
        }
        if (serveAsProxy) {
            alert('Please stop serving as a proxy first.')
            return
        }
                ...
                <div className="overflow-x-auto border border-gray-300 rounded-lg">
                    ...
                    <div>
                        ...
                        {listOfProxies && listOfProxies.length > 0 ? (
                            listOfProxies
                                .filter((proxy: Proxy) => proxy.peer_id !== currProxy?.peer_id)
                                .map((proxy: Proxy, index: number) => (
                                    <button
                                        key={index}
