...
import { deleteFile, getUploadedFiles, uploadFile, setWalletAddr } from '@renderer/rpcUtils'
...
function Upload(): JSX.Element {
    ...
    useEffect(() => {
        fetch('http://localhost:8080/account', {
            ...
            setWalletAddr(123, data.message).catch((e) => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            const filesToAppend = filesArr.map((file) => {
                byteCount += file.size
    ...
    const handleRemoveFile = async (index: number, file: fileType) => {
        const confirmed = window.confirm(`Are you sure you want to delete: ${file.file_name}?`)
        if (confirmed) {
            try {
